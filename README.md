...
织世录是一个多智能体叙事沙盘的前端原型，用来创建、查看和维护一个持续运行的虚构世界。当前版本包含水墨风工作台、新建世界流程，以及可在浏览器本地保存的世界设定编辑器。

## 功能概览

- 工作台首页：展示世界概览、运行状态、近期事件、最新章节、活跃角色、紧张关系、秘密和趋势。
- 新建世界：通过 `/worlds/new` 填写世界名称、类型、背景、规则、叙事风格和初始冲突。
- 世界设定编辑：在首页点击“世界设定”，编辑世界背景、规则、风格偏好、禁止事项和核心矛盾。
- 本地持久化：创建流程使用 `sessionStorage` 记录最近创建的世界，世界设定使用 `localStorage` 保存，刷新后仍可恢复。
...
提交成功后，首页顶部横幅会显示新世界名称、背景描述和标签；对应设定会写入浏览器本地存储。
...
保存后，首页顶部的世界描述和标签会立即更新；世界规则、禁止事项和核心矛盾只作为设定底稿保存，不会自动改写事件、角色关系、秘密或趋势模块。

### 3. 验证本地恢复

1. 创建或编辑一个世界。
2. 刷新首页，确认世界名称、背景和标签仍然保留。
3. 如需重置演示数据，可在浏览器开发者工具中清除本站的 `sessionStorage` 和 `localStorage`。
...
- 世界设定的读写、解析和映射逻辑位于 `src/lib/world-settings.ts`。
